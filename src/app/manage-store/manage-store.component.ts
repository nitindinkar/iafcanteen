...
export class ManageStoreComponent implements OnInit {
  ...
  addStore() {
    ...
    this.apiService
      .postApiWithToken(this.cons.api.addStore, jsonData)
      .subscribe({
        next: (v: object) => {
  ...
  activateStore(activeId: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.apiService
          .getApiWithToken(this.cons.api.activateStore + '/' + activeId)
          .subscribe(
            (response: object) => {
              let result: { [key: string]: any } = response;
              this.stores = result['response'];
  ...
  deactivateStore(deactiveId: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        // User confirmed, make API call
        this.apiService
          .getApiWithToken(this.cons.api.deactivateStore + '/' + deactiveId)
          .subscribe(
            (response: object) => {
              let result: { [key: string]: any } = response;
              this.stores = result['response'];
  ...
  deleteStore(delId: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.apiService
          .deleteApiWithToken(this.cons.api.deleteStore + '/' + delId)
          .subscribe({
            next: (response: any) => {
              ...
              if (
                response &&
                response.response !== 'exception Store is active' &&
                response.message === ''
              ) {
                Swal.fire({
                  icon: 'error',
  ...
  updateStore() {
    ...
    this.apiService
      .postApiWithToken(
        this.cons.api.updateSuperAdminStore + '/' + this.id,
        data
      )
      .subscribe(
        (response: any) => {
