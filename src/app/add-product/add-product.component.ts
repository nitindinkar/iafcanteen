...
export class AddProductComponent implements OnInit {
  ...
  deleteProduct(i: number, productId: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.apiService
          .deleteApiWithToken(this.cons.api.deleteProduct + '/' + productId)
          .subscribe(
            (response: object) => {
  ...
  searchProducts() {
    if (!this.searchQuery) {
      ...
      this.filteredProducts = this.products.filter(
        (product: { productName: string; category: { name: string } }) =>
          product.productName
            .toLowerCase()
  ...
  addProduct() {
    ...
    if (file) {
      ...
      this.apiService
        .postApiWithToken(this.cons.api.upload, formData)
        .subscribe({
          next: (v: object) => {
            let result: { [key: string]: any } = v;
            if (result['status'] == 200) {
  ...
  addProductWithImage() {
    ...
    this.apiService
      .postApiWithToken(this.cons.api.addProduct, jsonData)
      .subscribe({
        next: (v: object) => {
          let result: { [key: string]: any } = v;
          if (result['message'] == 'success') {
  ...
  updateProduct(uploadId: any) {
    ...
    this.apiService
      .postApiWithToken(this.cons.api.updateProduct, data)
      .subscribe(
        (response: any) => {
          let productDetails: any = response;
          this.edited = false;
