...
export class CheckoutComponent implements OnInit {
  ...
  bookOrder() {
    ...
    if (
      this.savedAddresses &&
      this.savedAddresses.length > selectedSavedAddressIndex
    ) {
      const selectedAddress = this.savedAddresses[selectedSavedAddressIndex];
      ...
      const billingData = {
        ...
        fullAddress:
          selectedAddress.flatNumberOrHouseNumber +
          ', ' +
      ...
      this.apiService
        .postApiWithToken(
          this.cons.api.buyProduct + '/' + this.flag,
          billingData
        )
        .subscribe({
          next: (v: object) => {
      ...
      console.error(
        'No saved addresses found or selected address index is out of bounds.'
      );
      Swal.fire({
        title: 'Error',
  ...
  getCartDetails() {
    ...
    this.apiService
      .getApiWithToken(this.cons.api.getCartDetailsOfUser + '/' + this.cardType)
      .subscribe(
        (response: object) => {
    ...
    if (this.cart && this.cart.length > 0) {
      for (let cartItem of this.cart) {
        if (
          ...
          this.subtotal +=
            cartItem.product.quantity * cartItem.product.productDiscountedPrice;
        }
      }
    }
    ...
    if (this.display) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          this.apiService
            .postApiWithToken(this.cons.api.saveAddress, addressDetails)
            .subscribe(
              (response: any) => {
  ...
  getAddress(): void {
    this.apiService
      .getApiWithToken(
        this.cons.api.getUserAddress + '/' + this.sharedService.userId
      )
      .subscribe(
        (response: any) => {
  ...
  toggleSelectSlot(address: any) {
    ...
    if (address) {
      ...
      console.log(
        'this is my selection ................' + this.showSelectSlot
      );
    } else {
  ...
  getStore() {
    ...
    this.apiService.getApiWithToken(this.cons.api.getAllStore).subscribe(
      (response: object) => {
        ...
        this.activeStores = this.stores
          .filter((store: { active: any }) => store.active)
          .map((store: { id: any; name: any }) => ({
            id: store.id,
            name: store.name,
          }));
