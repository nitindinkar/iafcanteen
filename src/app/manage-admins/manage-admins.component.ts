...
export class ManageAdminsComponent implements OnInit {
  ...
  addAdmins() {
    ...
    this.apiService
      .postApiWithToken(this.cons.api.addAdminDetails, jsonData)
      .subscribe({
        next: (v: object) => {
  ...
  activeAdmins(adminId: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.apiService
          .getApiWithToken(this.cons.api.activateAdmin + '/' + adminId)
          .subscribe(
            (response: object) => {
              let result: { [key: string]: any } = response;
  ...
  deactiveAdmins(adminId: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        // User clicked confirm button, make API call
        this.apiService
          .getApiWithToken(this.cons.api.deactivateAdmin + '/' + adminId)
          .subscribe(
            (response: object) => {
              let result: { [key: string]: any } = response;
  ...
  update() {
    ...
    this.apiService
      .postApiWithToken(this.cons.api.updateAdmin + '/' + this.id, data)
      .subscribe(
        (response: any) => {
  ...
  deleteAdmin(delAdminId: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.apiService
          .deleteApiWithToken(this.cons.api.deleteStore + '/' + delAdminId)
          .subscribe({
            next: (response: any) => {
  ...
  filterByAdmin(): void {
    ...
    if (this.selectedStatus === 'Filter By Admin Status') {
      ...
      this.filteredAdmin = this.allAdmins.filter(
