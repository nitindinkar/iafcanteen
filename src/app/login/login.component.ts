...
export class LoginComponent implements OnInit {
  ...
  registration() {
    ...
    this.HttpClient.post(registerationUrl, credentials, {
      responseType: 'text',
    }).subscribe(
      (response: any) => {
  ...
  login() {
    ...
    this.HttpClient.post(loginUrl, credentials).subscribe(
      (response: object) => {
        ...
        if (result['message'] == 'success') {
          ...
          localStorage.setItem(
            'userName',
            JSON.stringify(result['response']['user']['name'])
          );
          localStorage.setItem(
            'contactNumber',
            JSON.stringify(result['response']['user']['contactNumber'])
          );
          ...
          if (
            ...
            this.sharedService.cardType =
              this.ConstServiceService.constants.groceryCard;
            this.router.navigate(['']);
          } else if (
            this.sharedService.role != 'ADMIN' &&
            this.selected == 'Liquor'
          ) {
            localStorage.setItem(
              'card',
              this.ConstServiceService.constants.liquorCard
            );
            this.sharedService.cardType =
              this.ConstServiceService.constants.liquorCard;
  ...
  verifyOtp() {
    ...
    this.HttpClient.post(verifyOtpUrl, credentials, {
      responseType: 'text',
    }).subscribe(
      (response: any) => {
