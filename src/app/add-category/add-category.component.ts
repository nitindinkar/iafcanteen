...
export class AddCategoryComponent implements OnInit {
  ...
  autoSearch() {
    if (!this.searchQuery) {
      ...
      this.filteredProducts = this.categories.filter(
        (category: any) =>
          category.name
            .toLowerCase()
    ...
    this.apiService.postApiWithToken(this.cons.api.upload, formData).subscribe({
      next: (v: object) => {
        ...
        if (result['status'] == 200) {
          ...
          this.apiService
            .postApiWithToken(this.cons.api.addCategory, jsonData)
            .subscribe({
              next: (v: object) => {
                let result: { [key: string]: any } = v;
                if (result['message'] == 'success') {
  ...
  updateCategoryWithImage(uploadId: any) {
    ...
    this.apiService
      .updateApiWithTokenPatch(this.cons.api.updateCategory, data)
      .subscribe(
        (response: any) => {
